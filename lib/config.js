 ...
 */
exports.loadConfiguration = function loadConfiguration(filePath) {
    return readYaml(filePath).then((data) => {
        ...
        .then((data) => data.map((cfg, index) => {
            exports.basePath = path.posix.resolve(filePath, '..');
            const config = utils.mergeObjects(true, defaultConfig, cfg);
            validateConfiguration(config, index);
            return config;
        }));
};

function validateConfiguration(config, index) {
    const errors = [];
    const checkIt = (key) => {
