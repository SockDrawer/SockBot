...
function broadcastMessages(messages, callback) {
    ...
    setImmediate(() => callback(null, messages.filter(message => tester.test(message))));
}
...
function processTopics(topicMessages, callback) {
    ...
    topicMessages.forEach(message => {
        ...
        topics[topic].pus(message);
    });
    async.forEachOf(topics, (messages, topicId, next) => {
        browser.getTopic(topicId, (err, topic) => {
            if (err) {
                internals.events.emit('logWarning', 'Error Retrieving topic ' + topicId + ': ' + err);
                return next();
            }
            messages.forEach(message => {
                message.topic = topic;
...
function processPosts(postMessages, callback) {
    async.map(postMessages, (message, next) => {
        ...
        async.filter(messages, (message, filter) => {
            if (!message.post) {
                return filter(false);
            }
...
function handlemessages(messages) {
    broadcastMessages(messages, (_, topicMessages) => {
        processTopics(topicMessages, (__, postMessages) => {
            processPosts(postMessages, (___, finalMessages) => {
