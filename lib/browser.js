...
function throttleQueues(err, resp, waitTime) {
    const doubleTime = (minTime) => {
        const time = internals.pauseTime || 0;
        return Math.max(time * 2, minTime);
    };
    if (err || (resp && resp.statusCode >= 500)) {
        internals.pauseTime = doubleTime(6e4);
    } else if (waitTime >= 3000 || (resp && resp.statusCode >= 400)) {
        internals.pauseTime = doubleTime(3000);
    } else {
 ...
 * Get data from a URL
 *
 * @param {string} url URL to issue GET request to
 * @param {requestComplete} callback Completion callback
 */
 ...
 * Post data to a URL
 *
 * @param {string} url URL to issue POST request to
 * @param {object} data Data to POST
 * @param {requestComplete} callback Completion callback
 */
function postData(url, data, callback) {
    sendData.apply(this, ['POST', url, data, callback]);
}
/**
 * Put data to a URL
 *
 * @param {string} url URL to issue PUT request to
 * @param {object} data Data to PUT
 * @param {requestComplete} callback Completion callback
 */
function putData(url, data, callback) {
    sendData.apply(this, ['PUT', url, data, callback]);
}
...
function createPrivateMessage(to, title, content, callback) {
    ...
    this.queue.push({
        ...
        callback: (err, body) => {
            ...
            return callback(null, cleanPost(body));
        },
        delay: this.delay
    });
}
...
function getPost(postId, callback) {
    this.queue.push({
        ...
        callback: (err, post) => {
            ...
            return callback(null, cleanPost(post));
        },
        delay: this.delay
    });
}

function iterateClean(topic, eachPost, next){
    async.eachSeries(topic.post_stream.posts.map((p) => cleanPost(p)), (post, postNext) => {
...
function getPosts(topicId, eachPost, complete) {
    ...
    this.queue.push({
        ...
        callback: (err, topic) => {
            ...
            return async.whilst(() => posts.length > 0, (next) => {
                const part = [];
                ...
                ctx.queue.push({
                    ...
                    callback: (err2, topic2) => {
                        ...
                        return iterateClean(topic2, eachPost, next);
                    }
                });
            }, complete);
        }
 ...
 * @param {completionCallback} complete Completion callback
 */
function getLastPosts(topicId, eachPost, complete) {
    this.queue.push({
        method: 'GET',
        url: '/t/' + topicId + '/last.json?include_raw=1',
        callback: (err, topic) => {
            if (err) {
                return complete(err);
            }
            return iterateClean(topic, eachPost, complete);
        }
    });
}
        ...
        ctx.queue.push({
            ...
            callback: (err, topics) => {
                ...
                if (!topics.topic_list) {
                    return next(new Error('No topic list returned; unable to continue'));
                }
                url = topics.topic_list.more_topics_url;
                if (url.indexOf('.json') < 0) {
                    // It's a query string; there's only one '?'
                    url = url.replace('?', '.json?');
                }
                return async.eachSeries(topics.topic_list.topics, (topic, topicNext) => {
 ...
 *
 * @param {number} topicId Topic to alter notification level
 * @param {number} level Notification level to set (0=muted, 3=watching)
 * @param {function} callback Completion callback
 */
function setNotificationLevel(topicId, level, callback) {
    const ctx = this;
    ctx.queue.push({
        method: 'POST',
        url: '/t/' + topicId + '/notifications',
        delay: ctx.delay,
        form: {
