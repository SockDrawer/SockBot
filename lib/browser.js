...
function throttleQueues(err, resp, waitTime) {
    if (err || (resp && resp.statusCode > 400)) {
        if (internals.pauseTime > 0) {
            ...
            internals.pauseTime = 60 * 1000;
        }
    } else if (waitTime > 3 * 1000) {
        if (internals.pauseTime > 0) {
            internals.pauseTime *= 2;
        } else {
            internals.pauseTime = 3 * 1000;
        }
    } else {
        internals.pauseTime = 0;
    }
    if (internals.pauseTime > 10 * 60 * 1000) {
        internals.pauseTime = 10 * 60 * 1000;
    }
    if (internals.pauseTime > 0) {
        internals.pauseQueues(internals.pauseTime);
    }
}
...
function queueWorker(task, callback) {
    ...
    internals.request({
        ...
        internals.throttleQueues(err, resp, waitTime);
        try {
