...
exports.prepare = function prepare(events, callback) {
    ...
    events.on('loadPlugin', status.loadPlugin);
    async.parallel([
 ...
 *
 * @param {command} command The command that should be replied to as permission denied
 * @param {number|string} minimumTrust The minimum trust level required
 */
exports.postPermissionDenied = function postPermissionDenied(command, minimumTrust) {
    let trustMessage = 'a member of the forum staff';
    if (minimumTrust !== undefined) {
        if (typeof minimumTrust === 'number') {
            trustMessage = 'someone of Trust Level ' + minimumTrust + ' or higher';
        } else {
            trustMessage = '' + minimumTrust;
        }
    }
    browser.createPost(command.post.topic_id, command.post.post_number,
        ...
        'Please contact ' + trustMessage + ', or my owner, @' + config.core.owner +
...
function cmdShutUp(command) {
    if (command.post.trust_level < 6) { //gotta be staff+ to do this
        exports.postPermissionDenied(command);
