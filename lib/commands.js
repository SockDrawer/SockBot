    ...
    class Commands {
        ...
        static add(command, helpText, handler) {
            return new Promise((resolve, reject) => {
                ...
                const cmd = command.toLowerCase(),
                    shadowingMsg = `WARNING: alias ${command} is already registered, the alias will have no effect.`,
                    conflictMsg = `ERROR: command ${command} is already registered, cannot override existing command.`,
                    forbiddenMsg = `ERROR: command ${command} is not allowed by the active forum provider.`;
        ...
        static addAlias(command, handler) {
            return new Promise((resolve, reject) => {
                ...
                const cmd = command.toLowerCase(),
                    shadowedMsg = `WARNING: ${command} is already registered, this alias will have no effect.`,
                    shadowConflict = `ERROR: alias ${command} is already registered, cannot override existing alias.`,
                    forbiddenMsg = `ERROR: alias ${command} is not allowed by the active forum provider.`;
