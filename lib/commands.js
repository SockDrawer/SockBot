    ...
    class Commands {
        ...
        static add(command, helpText, handler) {
            return new Promise((resolve, reject) => {
                ...
                if (shadowHandlers[cmd]) {
                    const warn = `WARNING: alias ${command} is already registered, the alias will have no effect.`;
                    forum.emit('log', warn);
                }
                if (handlers[cmd]) {
                    const msg = `ERROR: command ${command} is already registered, cannot override existing command.`;
                    forum.emit('error', msg);
        ...
        static addAlias(command, handler) {
            return new Promise((resolve, reject) => {
                ...
                const cmd = command.toLowerCase();
                if (handlers[cmd]) {
                    forum.emit('log', `WARNING: ${command} is already registered, this alias will have no effect.`);
                }
                if (shadowHandlers[cmd]) {
                    const msg = `ERROR: alias ${command} is already registered, cannot override existing alias.`;
                    forum.emit('error', msg);
                    return reject('E_ALREADY_REGISTERED');
                }
