...
exports.prepare = function prepare(events, callback) {
    ...
    async.parallel([
        ...
        cb => events.onCommand('shutup', 'tell me to shutup', shutUp, cb)
    ], () => callback());
};
...
function shutupForReal() {
    internals.events.emit('logWarning', 'Shutting up by order!');
    SockBot.stop(() => {
        setTimeout(() => internals.events.emit('that was a long wait....'),
