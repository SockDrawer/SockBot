...
ElizaBot.prototype.reset = function() {
	...
	for (var k=0; k<elizadata.Keywords.length; k++) {
		this.lastchoice[k]=[];
		var rules=elizadata.Keywords[k][2];
...
ElizaBot.prototype._init = function() {
	...
	var synPatterns={};
	if ((elizadata.Synons) && (typeof elizadata.Synons == 'object')) {
		for (var i in elizadata.Synons) synPatterns[i]='('+i+'|'+elizadata.Synons[i].join('|')+')';
	}
	// check for keywords or install empty structure to prevent any errors
	if ((!elizadata.Keywords) || (typeof elizadata.Keywords.length == 'undefined')) {
