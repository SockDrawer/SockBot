...
posts in the target threads and like them, up to a maximum daily limit or until an error is recieved from 
discourse.
...
daily lile limit for the host forum.
...
## `scatter`
This option controls the amount of time the bot will scatter the likes by waiting an aditional period of 
